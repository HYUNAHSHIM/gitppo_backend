...
app.post(`/auth`, async (req, res) => {
  ...
  for await (let repo of repos.data) {
    const languageData = await axios.get(`https://api.github.com/repos/${user.data.login}/${repo.data.name}/languages`, {
      headers: {
        Authorization: `token ${token}`
      }
    });
    const readmeData = await axios.get(`https://api.github.com/repos/${user.data.login}/${repo.data.name}/contents/README.md`, {
      headers: {
        Authorization: `token ${token}`
      }
    });
